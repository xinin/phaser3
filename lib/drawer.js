...
class Drawer {
  static fillScene(scene, config) {
    ...
    config.staticObjects.forEach((obj) => {
      ...
      mapObjects.staticObjects[obj.id] = new DynamicReference(scene, obj);
      mapObjects.staticObjects[obj.id].draw();
      scene.physics.add.collider(mapObjects.Player.sprite, mapObjects.staticObjects[obj.id].sprite);
    });
